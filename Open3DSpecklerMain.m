...
%
%   Description: 3D-Speckler is a MATLAB-based software used for the 
%                quantification of locations, sizes, and intensities 
%                of fluorescent particles. 3D-Aligner is built into 
%                3D-Speckler as a package to correct for sample drift
%                during image acquisition for 3D image reconstruction.
%
%   Last Edited:        October 18th, 2023
%   Last Edited By:     Jonathan Loi
%   Version:            1.0.5
%
%   Version Notes:      Version for submission for 3D-Aligner manuscript.
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
global  fullPath nChannels nPlanes imgSize zSlice xyRes zRes chOptions...
        ...
        sortFittedDataBox correctedDistances driftVec showDrift saveDrift...
        nnDrift correctedDistance488 correctedDistance594 correctedDistance640...
        correctedDistance405 driftIMG488 driftIMG594 driftIMG640 driftIMG405...
        avgCorrectedDistances driftCorrVec driftCheck corrVec...
        distSTD488 distN488 distSTD594 distN594 distSTD640 distN640 distSTD405 distN405...
        pixelThresholdMax pixelThresholdMin deComp488 deComp594 deComp640 deComp405
...
function openMainUI()
    ...
    modeOptions = uicontrol('Parent', mainFig,'Style','popupmenu','Position',[viewFigSize(1)*0.81, viewFigSize(2)*0.94,200,20],...
            ...
            , 'Chromatic Aberration Calibration', '3D-Aligner Drift Correction'
...
%             particleAnalysisSetup();
        ...
        elseif modeChoice == 5
            ...
            if (nPlanes > 1) && (nnz(nOptions) >= 2)
                driftCorrectionSetup();
            else
                warndlg('Error: Drift Correction requires a 3D image and 2 or more channels.');
                return;
            end
        end
    ...
    function driftCorrectionSetup()
        
        global driftVec showDrift saveDrift driftCorrVec driftCheck...
                correctedDistance488 correctedDistance594 correctedDistance640...
                    correctedDistance405 driftIMG488 driftIMG594...
                    driftIMG640 driftIMG405 avgCorrectedDistances...
                    pixelThresholdMax pixelThresholdMin nnDrift
        
                uicontrol('Parent', mainFig, 'Style', 'pushbutton', 'Position', [viewFigSize(1)*0.815, viewFigSize(2)*0.83,180, 35],...
                    'String', 'Determine Drift', 'BackgroundColor',[1,0.7,0.4], 'FontSize', 11, 'Callback', {@startAlignment});
                uicontrol('Parent', mainFig, 'Style', 'pushbutton', 'Position', [viewFigSize(1)*0.815, viewFigSize(2)*0.05,180, 35],...
                    'String', 'Correct & Export Images', 'BackgroundColor',[0.8,0.8,1], 'FontSize', 10, 'Callback', {@performAlignment});
                
                driftCheck = 0;
                % Right Side Panel %
                driftPan = uibuttongroup('Title','Drift Correction Options','FontSize',8,...
                    'BackgroundColor',[1,1,1],'FontWeight', 'bold',...
                    'Position',[0.815 0.64 .15 .15]);
                showDriftOpt = uicontrol(driftPan,'Style','Checkbox','String','  Display Drift Results','FontSize',9,...
                    'pos',[15 72 200 30],'BackgroundColor', [1,1,1]);
                saveDriftOpt = uicontrol(driftPan,'Style','Checkbox','String','  Save Drift Plots','FontSize',9,...
                    'pos',[15 50 200 30],'BackgroundColor', [1,1,1]);
                
                % Left Side Instructions %
                annotation('textbox', [0.015, 0.65, 0.1, 0.1], 'string', 'ExM Drift Image Correction', 'FontSize', 11, 'fontweight', 'bold', 'EdgeColor', 'none', 'Color', [0, 0, 0]);
                
                annotation('textbox', [0.025, 0.64, 0.25, 0.05], 'string', '- Choose pixel size threshold', 'FontSize', 10, 'fontweight', 'bold', 'EdgeColor', 'none');
                annotation('textbox', [0.025, 0.62, 0.25, 0.05], 'string', '  for feature detection below.', 'FontSize', 10, 'fontweight', 'bold', 'EdgeColor', 'none');
                
                annotation('textbox', [0.025, 0.57, 0.25, 0.05], 'string', '- Choose one of more channels', 'FontSize', 10, 'fontweight', 'bold', 'EdgeColor', 'none');
                annotation('textbox', [0.025, 0.55, 0.25, 0.05], 'string', '  for feature extraction.', 'FontSize', 10, 'fontweight', 'bold', 'EdgeColor', 'none');
                
                annotation('textbox', [0.025, 0.50, 0.25, 0.05], 'string', '- "Display Drift Results" will', 'FontSize', 10, 'fontweight', 'bold', 'EdgeColor', 'none');
                annotation('textbox', [0.025, 0.48, 0.25, 0.05], 'string', '  plot drift throughout Z stack.', 'FontSize', 10, 'fontweight', 'bold', 'EdgeColor', 'none');
                annotation('textbox', [0.025, 0.46, 0.25, 0.05], 'string', '  to determine channel for most ', 'FontSize', 10, 'fontweight', 'bold', 'EdgeColor', 'none');
                annotation('textbox', [0.025, 0.44, 0.25, 0.05], 'string', '  accurate drift correction. ', 'FontSize', 10, 'fontweight', 'bold', 'EdgeColor', 'none');
                
                annotation('textbox', [0.025, 0.39, 0.25, 0.05], 'string', '- Multiple channels chosen will', 'FontSize', 10, 'fontweight', 'bold', 'EdgeColor', 'none');
                annotation('textbox', [0.025, 0.37, 0.25, 0.05], 'string', '  result in average of channels', 'FontSize', 10, 'fontweight', 'bold', 'EdgeColor', 'none');
                annotation('textbox', [0.025, 0.35, 0.25, 0.05], 'string', '  for final drift correction.', 'FontSize', 10, 'fontweight', 'bold', 'EdgeColor', 'none');
                
                annotation('textbox', [0.025, 0.30, 0.25, 0.05], 'string', '- Corrected images will be', 'FontSize', 10, 'fontweight', 'bold', 'EdgeColor', 'none');
                annotation('textbox', [0.025, 0.28, 0.25, 0.05], 'string', '  exported and saved into a', 'FontSize', 10, 'fontweight', 'bold', 'EdgeColor', 'none');
                annotation('textbox', [0.025, 0.26, 0.25, 0.05], 'string', '  folder in file location as TIF.', 'FontSize', 10, 'fontweight', 'bold', 'EdgeColor', 'none');
                
                annotation('textbox', [0.044, 0.16, 0.1, 0.1], 'string', 'FEATURE SIZE FILTER', 'FontSize', 9, 'fontweight', 'bold', 'EdgeColor', 'none', 'Color', [0.8, 0.4, 0]);
                annotation('textbox', [0.027, 0.18, 0.1, 0.05], 'string', 'Lower Limit (pxls)', 'FontSize', 7, 'fontweight', 'bold', 'EdgeColor', 'none');
                annotation('textbox', [0.112, 0.18, 0.1, 0.05], 'string', 'Upper Limit (pxls)', 'FontSize', 7, 'fontweight', 'bold', 'EdgeColor', 'none');
                
                if isempty(pixelThresholdMin)
                    pixelThresholdMin = 10;
                end
                if isempty(pixelThresholdMax)
                    pixelThresholdMax = 60;
                end
                
                sizeLower2 = uicontrol(mainFig,'Style','edit','String',num2str(pixelThresholdMin),'FontSize',8,...
                    'pos',[viewFigSize(1)*0.034, viewFigSize(2)*0.18 80 20], 'BackgroundColor',[1,1,1]);
                sizeUpper2 = uicontrol(mainFig,'Style','edit','String',num2str(pixelThresholdMax),'FontSize',8,...
                    'pos',[viewFigSize(1)*0.118, viewFigSize(2)*0.18 80 20], 'BackgroundColor',[1,1,1]);
                uicontrol('Parent', mainFig, 'Style', 'pushbutton', 'Position', [viewFigSize(1)*0.045, viewFigSize(2)*0.14, 150, 20],...
                    'String', 'Update Feature Size Limits', 'BackgroundColor',[0.7,0.7,0.7], 'FontSize', 8, 'Callback', {@updateFeatureSize});

        function updateFeatureSize(~,~)
            
            pixelThresholdMin = str2double(get(sizeLower2, 'String'));
            pixelThresholdMax = str2double(get(sizeUpper2, 'String'));
            disp('.....................................');
            disp('New Feature Size Limits Set');
            disp('.');
            disp(strcat('Upper Size Limit: ', num2str(pixelThresholdMin)));
            disp(strcat('Lower Size Limit: ', num2str(pixelThresholdMax)));
            disp('.....................................');
        end
        ...
        function performAlignment(~,~)
            ...
            if driftCheck ~= 1
                warndlg('Error: Please determine drift correction values before correcting images.');
                return;
            end

            saveDrift = get(saveDriftOpt, 'Value');
            
            driftFig = figure();
            set(gcf,'NumberTitle','off')
            set(gcf,'Name','3D-Aligner Drift Correction')
            clf;
            set(gcf, 'Position',  [100, 100, 375, 350]);
            set(gcf, 'Resize', 'off');
            driftPan = uibuttongroup('Title','Choose Channels For Drift Detection','FontSize',12,...
                'BackgroundColor',[1,1,1],'FontWeight', 'bold',...
            ...
            if driftVec(1) ~= 0
                opt488 = uicontrol(driftPan,'Style','Checkbox','String','Green Channel','FontSize',12,...
            ...
            if driftVec(2) ~= 0
                opt594 = uicontrol(driftPan,'Style','Checkbox','String','Red Channel','FontSize',12,...
            ...
            if driftVec(3) ~= 0
                opt640 = uicontrol(driftPan,'Style','Checkbox','String','Far Red Channel','FontSize',12,...
            ...
            if driftVec(4) ~= 0
                opt405 = uicontrol(driftPan,'Style','Checkbox','String','Blue Channel','FontSize',12,...
            ...
            uicontrol('Parent', driftFig, 'Style', 'pushbutton', 'Position', [200,20,150, 35],...
                'FontSize',16, 'String', 'CONFIRM', 'Callback', {@setDriftCorrection2});

            function setDriftCorrection2(~,~)
                ...
                driftCorrVec = [ch488 ch594 ch640 ch405];

                if sum(driftCorrVec) >= 1
                    ...
                    warndlg('Error: Please choose channel(s) for drift correction.');
                    return;
                end
        ...
        function startAlignment(~,~)
            ...
            set(gcf, 'Position',  [100, 100, 375, 350]);
            set(gcf, 'Resize', 'off');
            driftPan = uibuttongroup('Title','Choose Channels For Feature Detection','FontSize',12,...
                'BackgroundColor',[1,1,1],'FontWeight', 'bold',...
                'Position',[0.05 0.2 .9 .7]);
            ...
            showDrift = get(showDriftOpt, 'Value');
            saveDrift = get(saveDriftOpt, 'Value');
            ...
            uicontrol('Parent', driftFig, 'Style', 'pushbutton', 'Position', [200,20,150, 35],...
                'FontSize',16, 'String', 'CONFIRM', 'Callback', {@setDriftCorrection});

            function setDriftCorrection(~,~)
                ...
                driftVec = [ch488 ch594 ch640 ch405];
                
                prompt = {'Choose Feature Matching Distance (pixels)'};
                dlgtitle = 'Nearest Neighbor Distance';
                dims = [1 35];
                definput = {'10'};
                ...
                disp(strcat('Nearest Neighbor Feature Matching Distance: *', answer));
                nnDrift = str2double(answer);
                ...
                if sum(driftVec) >= 1
                    ...
                    warndlg('Error: Please choose channel(s) for feature extraction.');
                    return;
                end
...
function performDriftCorrection()
    ...
    global  showDrift saveDrift nOptions driftVec nnDrift channel1...
        channel2 channel3 channel4 correctedDistance488 correctedDistance594...
        correctedDistance640 correctedDistance405 driftIMG488 driftIMG594...
        driftIMG640 driftIMG405 avgCorrectedDistances xyRes nPlanes driftCheck...
        distSTD488 distN488 distSTD594 distN594 distSTD640 distN640 distSTD405 distN405...
        deComp488 deComp594 deComp640 deComp405

    disp('****************************************************************');
    disp('Total Channels Chosen for Drift Determination:');
    if driftVec(1) == 1
        disp('Green');
    end
    if driftVec(2) == 1
    ...
    % Determine Drift and Plot in Green Channel %
    if driftVec(1) == 1
        ...
        disp('Currently Determining Drift in Green Channel:');
        try
            [correctedDistance488, distSTD488, distN488, deComp488] = extractDrift(driftIMG);
            if showDrift == 1
                
                driftData = correctedDistance488;
                figure();
                clf; hold on;
                set(gcf, 'Position', [100, 100, 1200,600]);
                x = 1:size(driftData,1);
                x = x';
                y = driftData(:,1);
                y(:,1) = (y(:,1)-y(1,1))*xyRes;
                grid on;
                xlim([0 nPlanes]);
                x2 = 1:size(driftData,1);
                x2 = x2';
                y2 = driftData(:,2);
                y2(:,1) = (y2(:,1)-y2(1,1))*xyRes;
                h(1) = scatter(x,y);
                h(2) = scatter(x2,y2);
                h(3) = plot(x,y);
                h(4) = plot(x2,y2);
                ...
                legend(h(1:2), 'X Drift', 'Y Drift', 'Location', 'eastoutside')
                title('Drift Across Z-Stack - Green Channel');
                xlabel('Z Slice');
                ylabel('Drift (um)');
                
            end
        catch
            warndlg('Error: Green channel drift cannot properly be determined.');
            disp('Error: Green channel drift cannot properly be determined.');
            disp('****************************************************************');
            driftVec(1) = 0;
        end
    ...
    % Determine Drift and Plot in Red Channel %
    if driftVec(2) == 1
        ...
        disp('Currently Determining Drift in Red Channel:');
        try
            [correctedDistance594, distSTD594, distN594, deComp594] = extractDrift(driftIMG);
            if showDrift == 1
                
                driftData = correctedDistance594;
                figure();
                clf; hold on;
                set(gcf, 'Position', [100, 100, 1200,600]);
                x = 1:size(driftData,1);
                ...
                set(gca,'FontSize',24) % Creates an axes and sets its FontSize to 18
                legend(h(1:2), 'X Drift', 'Y Drift', 'Location', 'eastoutside')
                title('Drift Across Z-Stack - Red Channel');
                xlabel('Z Slice');
                ylabel('Drift (um)');
                
            end
        catch
            warndlg('Error: Red channel drift cannot properly be determined.');
            disp('Error: Red channel drift cannot properly be determined.');
            disp('****************************************************************');
            driftVec(2) = 0;
        end
    ...
    % Determine Drift and Plot in Far Red Channel %
    if driftVec(3) == 1
        ...
        disp('Currently Determining Drift in Far Red Channel:');
        try
            [correctedDistance640, distSTD640, distN640, deComp640] = extractDrift(driftIMG);
            if showDrift == 1
                
                driftData = correctedDistance640;
                figure();
                clf; hold on;
                set(gcf, 'Position', [100, 100, 1200,600]);
                x = 1:size(driftData,1);
                ...
                set(gca,'FontSize',24) % Creates an axes and sets its FontSize to 18
                legend(h(1:2), 'X Drift', 'Y Drift', 'Location', 'eastoutside')
                title('Drift Across Z-Stack - Far Red Channel');
                xlabel('Z Slice');
                ylabel('Drift (um)');
                
            end
        catch
            warndlg('Error: Far Red channel drift cannot properly be determined.');
            disp('Error: Far Red channel drift cannot properly be determined.');
            disp('****************************************************************');
            driftVec(3) = 0;
        end
    ...
    % Determine Drift and Plot in Blue Channel %
    if driftVec(4) == 1
        ...
        disp('Currently Determining Drift in Blue Channel:');
        try
            [correctedDistance405, distSTD405, distN405, deComp405] = extractDrift(driftIMG);
            if showDrift == 1
                
                driftData = correctedDistance405;
                figure();
                clf; hold on;
                set(gcf, 'Position', [100, 100, 1200,600]);
                x = 1:size(driftData,1);
                ...
                set(gca,'FontSize',24) % Creates an axes and sets its FontSize to 18
                legend(h(1:2), 'X Drift', 'Y Drift', 'Location', 'eastoutside')
                title('Drift Across Z-Stack - Blue Channel');
                xlabel('Z Slice');
                ylabel('Drift (um)');
                
            end
        catch
            warndlg('Error: Blue channel drift cannot properly be determined.');
            disp('Error: Blue channel drift cannot properly be determined.');
            disp('****************************************************************');
            driftVec(4) = 0;
        end
    ...
    function [correctedDistance, stdVec, nVec, deCompVec] = extractDrift(driftIMG)

        correctedDistance = zeros(size(driftIMG,3), 2);
        correctedDistance(1,1) = 0;
        correctedDistance(1,2) = 0;
        
        deCompVec = zeros(size(driftIMG,3), 2);
        deCompVec(1,1) = 0;
        deCompVec(1,2) = 0;
        
        stdVec = zeros(size(driftIMG,3), 2);
        stdVec(1,1) = 0;
        stdVec(1,2) = 0;
        
        nVec = zeros(size(driftIMG,3), 2);
        nVec(1,1) = 0;
        nVec(1,2) = 0;
        ...
        for i = 1:size(driftIMG,3)-1

            image1 = uint16(driftIMG(:,:,i));
            image2 = uint16(driftIMG(:,:,i+1));

            try
                [xDistance,yDistance,sx,sy,nx,ny] = compareImages(image1,image2);
            catch
                xDistance = 0;
                yDistance = 0;
                sx = 0;
            ...
            correctedDistance(i+1,1) = xSum;
            correctedDistance(i+1,2) = ySum;
            
            deCompVec(i+1,1) = xDistance;
            deCompVec(i+1,2) = yDistance;
            ...
            if i == size(driftIMG,3)-1
                disp(strcat('Current Progress: 100%'));
            elseif mod(i,5) == 0
                perc = 100*i/size(driftIMG,3);
                str1 = strcat(num2str(round(perc,1,'decimals')),'%');
                disp(strcat('Current Progress: ',str1));
            end
    ...
    function [xDistance, yDistance,sx,sy,nx,ny] = compareImages(image1,image2)
        % process image %
        locations1 = process(image1);
        locations2 = process(image2);

        % match points %
        [matchedPts1, matchedPts2] = matchPointsDrift(locations1, locations2, nnDrift);

        points1 = cornerPoints(matchedPts1);
        points2 = cornerPoints(matchedPts2);
        position1 = points1.Location;
        position2 = points2.Location;

        xDir = [];
        yDir = [];

        for i = 1:size(position1,1)
            xDif = position2(i,1)-position1(i,1);
            yDif = position2(i,2)-position1(i,2);
            xDir = [xDir xDif];
            yDir = [yDir yDif];
        end

%         filtData = rmoutliers([xDir yDir]);
%         xDir = filtData(:,1);
%         yDir = filtData(:,2);
        
        xDistance = median(xDir,'all');
        yDistance = median(yDir,'all');
    ...
    function centroids = process(image)
        
        global pixelThresholdMax pixelThresholdMin
        
        warning off
        BW1 = edge(image,'log');
        BW1 = imclose(BW1,ones(2,2));
        BW1 = bwmorph(BW1,'bridge');
        BW1 = bwpropfilt(BW1,image,'MeanIntensity', 200);
        BW1 = imfill( BW1, 'holes' );
        BW2 = imopen(BW1, ones(2,2));
               
        BW3 = bwareaopen(BW2, pixelThresholdMin) & ~bwareaopen(BW2,pixelThresholdMax);
    ...
    function [newData1,newData2] = matchPointsDrift(data1, data2, nnThreshold)
...
function driftCorrectIMGs()

    global channel1 channel2 channel3 channel4 nOptions correctedDistance488 correctedDistance594...
        correctedDistance640 correctedDistance405 avgCorrectedDistances nPlanes driftCorrVec...
        fullPath corrVec exportPath saveDrift xyRes deComp488 deComp594 deComp640 deComp405

    % Determine Final Correction Vector %
    if sum(driftCorrVec) == 1

        if driftCorrVec(1) == 1
            corrVec = correctedDistance488;
        elseif driftCorrVec(2) == 1
            corrVec = correctedDistance594;
        elseif driftCorrVec(3) == 1
            corrVec = correctedDistance640;
        elseif driftCorrVec(4) == 1
            corrVec = correctedDistance405;
        end

    elseif sum(driftCorrVec) > 1

        if (driftCorrVec(1) == 1) && (driftCorrVec(2) == 1) && (driftCorrVec(3) == 0) && (driftCorrVec(4) == 0)
            avgCorrectedDistances = nonzeroAVG2(deComp488, deComp594);
        elseif (driftCorrVec(1) == 1) && (driftCorrVec(2) == 0) && (driftCorrVec(3) == 1) && (driftCorrVec(4) == 0)
            avgCorrectedDistances = nonzeroAVG2(deComp488, deComp640);
        elseif (driftCorrVec(1) == 1) && (driftCorrVec(2) == 0) && (driftCorrVec(3) == 0) && (driftCorrVec(4) == 1)
            avgCorrectedDistances = nonzeroAVG2(deComp488, deComp405);
        elseif (driftCorrVec(1) == 0) && (driftCorrVec(2) == 1) && (driftCorrVec(3) == 1) && (driftCorrVec(4) == 0)
            avgCorrectedDistances = nonzeroAVG2(deComp594, deComp640);
        elseif (driftCorrVec(1) == 0) && (driftCorrVec(2) == 1) && (driftCorrVec(3) == 0) && (driftCorrVec(4) == 1)
            avgCorrectedDistances = nonzeroAVG2(deComp594, deComp405);
        elseif (driftCorrVec(1) == 0) && (driftCorrVec(2) == 0) && (driftCorrVec(3) == 1) && (driftCorrVec(4) == 1)
            avgCorrectedDistances = nonzeroAVG2(deComp640, deComp405);
        elseif (driftCorrVec(1) == 1) && (driftCorrVec(2) == 1) && (driftCorrVec(3) == 1) && (driftCorrVec(4) == 0)
            avgCorrectedDistances = nonzeroAVG3(deComp488, deComp594, deComp640);
        elseif (driftCorrVec(1) == 1) && (driftCorrVec(2) == 1) && (driftCorrVec(3) == 0) && (driftCorrVec(4) == 1)
            avgCorrectedDistances = nonzeroAVG3(deComp488, deComp594, deComp405);
        elseif (driftCorrVec(1) == 1) && (driftCorrVec(2) == 0) && (driftCorrVec(3) == 1) && (driftCorrVec(4) == 1)
            avgCorrectedDistances = nonzeroAVG3(deComp488, deComp640, deComp405);
        elseif (driftCorrVec(1) == 0) && (driftCorrVec(2) == 1) && (driftCorrVec(3) == 1) && (driftCorrVec(4) == 1)
            avgCorrectedDistances = nonzeroAVG3(deComp594, deComp640, deComp405);
        elseif (driftCorrVec(1) == 1) && (driftCorrVec(2) == 1) && (driftCorrVec(3) == 1) && (driftCorrVec(4) == 1)
            avgCorrectedDistances = nonzeroAVG4(deComp488, deComp594, deComp640, deComp405);
        end

        corrVec = cumsum(avgCorrectedDistances);

    end
   
    [fp,fname] = fileparts(fullPath);
    
    expTempName = strcat('DriftResults_', fname);
    [baseFileName, folderExp] = uiputfile(strcat(expTempName, '.xlsx'), 'Choose File Name');
    ...
    newDirName = strcat('DRIFT_RESULTS_', fname);
    mkdir(fullfile(folderExp,newDirName));
    exportPath = fullfile(strcat(folderExp,newDirName), baseFileName);
    
    testVec = sum([~isempty(correctedDistance488) ~isempty(correctedDistance594) ~isempty(correctedDistance640) ~isempty(correctedDistance405)]);
    colNames = {'Drift_X','Drift_Y'};
    
    disp('**************************************************************');
    disp('Exporting Saved Drift Data to Excel...');
    
    % Export Final Correction Values %
    if ~isempty(corrVec) && (testVec > 1)
        
        finalT = array2table(corrVec*xyRes, 'VariableNames', colNames);
        
        disp('Saving Final Drift Correction Data to Excel...');
        warning('off', 'MATLAB:xlswrite:AddSheet');
        writetable(finalT, exportPath, 'Sheet', 'Final_Drift_Values');
        
        if saveDrift == 1
            
            disp('Saving Final Drift Plots...');
            driftData = corrVec;
            visFig = figure();
            clf; hold on;
            set(gcf, 'Position', [100, 100, 1200,600]);
            ...
            xlim([0 nPlanes]);
            set(gca,'FontSize',24) % Creates an axes and sets its FontSize to 18
            legend(h(1:2), 'X Drift', 'Y Drift', 'Location', 'northwest')
            title('Drift Across Z-Stack - Final Drift Correction');
            xlabel('Z Slice');
            ylabel('Drift (um)');
            
            saveas(visFig, fullfile(fullfile(folderExp,newDirName),strcat(fname,'_FinalDriftPlot')));
            close(visFig);
    ...
    % Export Drift 488 %
    if ~isempty(correctedDistance488)
        
        finalT488 = array2table(correctedDistance488*xyRes, 'VariableNames', colNames);
        
        disp('Saving Green Channel Drift Data to Excel...');
        warning('off', 'MATLAB:xlswrite:AddSheet');
        writetable(finalT488, exportPath, 'Sheet', 'Drift_Green');
        
        if saveDrift == 1
            
            disp('Saving Green Channel Drift Plots...');
            driftData = correctedDistance488;
            visFig = figure();
            clf; hold on;
            set(gcf, 'Position', [100, 100, 1200,600]);
            ...
            set(gca,'FontSize',24) % Creates an axes and sets its FontSize to 18
            legend(h(1:2), 'X Drift', 'Y Drift', 'Location', 'northwest')
            title('Drift Across Z-Stack - Green Channel Drift');
            xlabel('Z Slice');
            ylabel('Drift (um)');
            
            saveas(visFig, fullfile(fullfile(folderExp,newDirName),strcat(fname,'_GreenDriftPlot')));
            close(visFig);
    ...
    % Export Drift 594 %
    if ~isempty(correctedDistance594)
        
        finalT594 = array2table(correctedDistance594*xyRes, 'VariableNames', colNames);
        
        disp('Saving Red Channel Drift Data to Excel...');
        warning('off', 'MATLAB:xlswrite:AddSheet');
        writetable(finalT594, exportPath, 'Sheet', 'Drift_Red');
        
        if saveDrift == 1
            
            disp('Saving Red Channel Drift Plots...');
            driftData = correctedDistance594;
            visFig = figure();
            clf; hold on;
            set(gcf, 'Position', [100, 100, 1200,600]);
            ...
            set(gca,'FontSize',24) % Creates an axes and sets its FontSize to 18
            legend(h(1:2), 'X Drift', 'Y Drift', 'Location', 'northwest')
            title('Drift Across Z-Stack - Red Channel Drift');
            xlabel('Z Slice');
            ylabel('Drift (um)');
            
            saveas(visFig, fullfile(fullfile(folderExp,newDirName),strcat(fname,'_RedDriftPlot')));
            close(visFig);
    ...
    % Export Drift 640 %
    if ~isempty(correctedDistance640)
        
        finalT640 = array2table(correctedDistance640*xyRes, 'VariableNames', colNames);
        
        disp('Saving Far Red Channel Drift Data to Excel...');
        warning('off', 'MATLAB:xlswrite:AddSheet');
        writetable(finalT640, exportPath, 'Sheet', 'Drift_FarRed');
        
        if saveDrift == 1
            
            disp('Saving Far Red Channel Drift Plots...');
            driftData = correctedDistance640;
            visFig = figure();
            clf; hold on;
            set(gcf, 'Position', [100, 100, 1200,600]);
            ...
            set(gca,'FontSize',24) % Creates an axes and sets its FontSize to 18
            legend(h(1:2), 'X Drift', 'Y Drift', 'Location', 'northwest')
            title('Drift Across Z-Stack - Far Red Channel Drift');
            xlabel('Z Slice');
            ylabel('Drift (um)');
            
            saveas(visFig, fullfile(fullfile(folderExp,newDirName),strcat(fname,'_FarRedDriftPlot')));
            close(visFig);
    ...
    % Export Drift 405 %
    if ~isempty(correctedDistance405)
        
        finalT405 = array2table(correctedDistance405*xyRes, 'VariableNames', colNames);
        
        disp('Saving Blue Channel Drift Data to Excel...');
        warning('off', 'MATLAB:xlswrite:AddSheet');
        writetable(finalT405, exportPath, 'Sheet', 'Drift_Blue');
        
        if saveDrift == 1
            
            disp('Saving Blue Channel Drift Plots...');
            driftData = correctedDistance405;
            visFig = figure();
            clf; hold on;
            set(gcf, 'Position', [100, 100, 1200,600]);
            ...
            set(gca,'FontSize',24) % Creates an axes and sets its FontSize to 18
            legend(h(1:2), 'X Drift', 'Y Drift', 'Location', 'northwest')
            title('Drift Across Z-Stack - Blue Channel Drift');
            xlabel('Z Slice');
            ylabel('Drift (um)');
            
            saveas(visFig, fullfile(fullfile(folderExp,newDirName),strcat(fname,'_BlueDriftPlot')));
            close(visFig);
    ...
    disp('Drift Data Successfully Exported to Excel.');
    disp('**************************************************************');
    
    [fp2,fname2] = fileparts(exportPath);
    
    newDirName = strcat('DriftCorrectedIMGs_', fname);
    mkdir(fullfile(fp2,newDirName));
    imgExportPath = fullfile(fp2,newDirName);
    
    % Create Image Export Paths
    exportPathG = fullfile(imgExportPath,strcat(fname,'_DriftCorrected_Green.tif'));
    exportPathR = fullfile(imgExportPath,strcat(fname,'_DriftCorrected_Red.tif'));
    exportPathFR = fullfile(imgExportPath,strcat(fname,'_DriftCorrected_FarRed.tif'));
    exportPathB = fullfile(imgExportPath,strcat(fname,'_DriftCorrected_Blue.tif'));
    
    disp('*******************************************************');
    disp('Performing Drift Correction on existing images.');
    
    % EXPORT ALL DRIFT CORRECTED IMAGES FIRST %
 
    % Export Corrected Green IMG %
    if (nOptions(1) ~= 0)
        disp('Correcting Green Channel...');
        if nOptions(1) == 1
            expDriftIMG = channel1;
        elseif nOptions(1) == 2
            expDriftIMG = channel2;
        elseif nOptions(1) == 3
            expDriftIMG = channel3;
        elseif nOptions(1) == 4
            expDriftIMG = channel4;
        end
        imwrite(uint16(expDriftIMG(:,:,1)), exportPathG, 'WriteMode', 'append', 'Compression', 'none');
        for i = 1:size(expDriftIMG,3)-1
            image2 = uint16(expDriftIMG(:,:,i+1));
            correctedIMG_Exp = imtranslate(image2, [-corrVec(i+1,1),-corrVec(i+1,2)]);
            imwrite(correctedIMG_Exp, exportPathG, 'WriteMode', 'append', 'Compression', 'none');
            if i == nPlanes-1
                disp(strcat('Current Progress: 100%'));
            elseif mod(i,5) == 0
                perc = 100*i/nPlanes;
        ...
        disp('Green Channel Drift Correction and IMG Export Successful.');
    end
    
    % Export Corrected Red IMG %
    if (nOptions(2) ~= 0)
        disp('Correcting Red Channel...');
        if nOptions(2) == 1
            expDriftIMG = channel1;
        elseif nOptions(2) == 2
            expDriftIMG = channel2;
        ...
        imwrite(uint16(expDriftIMG(:,:,1)), exportPathR, 'WriteMode', 'append', 'Compression', 'none');
        for i = 1:size(expDriftIMG,3)-1
            image2 = uint16(expDriftIMG(:,:,i+1));
            correctedIMG_Exp = imtranslate(image2, [-corrVec(i+1,1),-corrVec(i+1,2)]);
            imwrite(correctedIMG_Exp, exportPathR, 'WriteMode', 'append', 'Compression', 'none');
            if i == nPlanes-1
                disp(strcat('Current Progress: 100%'));
            elseif mod(i,5) == 0
                perc = 100*i/nPlanes;
        ...
        disp('Red Channel Drift Correction and IMG Export Successful.');
    end
    
    % Export Corrected Far Red IMG %
    if (nOptions(3) ~= 0)
        disp('Correcting Far Red Channel...');
        if nOptions(3) == 1
            expDriftIMG = channel1;
        elseif nOptions(3) == 2
            expDriftIMG = channel2;
        ...
        imwrite(uint16(expDriftIMG(:,:,1)), exportPathFR, 'WriteMode', 'append', 'Compression', 'none');
        for i = 1:size(expDriftIMG,3)-1
            image2 = uint16(expDriftIMG(:,:,i+1));
            correctedIMG_Exp = imtranslate(image2, [-corrVec(i+1,1),-corrVec(i+1,2)]);
            imwrite(correctedIMG_Exp, exportPathFR, 'WriteMode', 'append', 'Compression', 'none');
            if i == nPlanes-1
                disp(strcat('Current Progress: 100%'));
            elseif mod(i,5) == 0
                perc = 100*i/nPlanes;
        ...
        disp('Far Red Channel Drift Correction and IMG Export Successful.');
    end
    
    % Export Corrected Blue IMG %
    if (nOptions(4) ~= 0)
        disp('Correcting Blue Channel...');
        if nOptions(4) == 1
            expDriftIMG = channel1;
        elseif nOptions(4) == 2
            expDriftIMG = channel2;
        ...
        imwrite(uint16(expDriftIMG(:,:,1)), exportPathB, 'WriteMode', 'append', 'Compression', 'none');
        for i = 1:size(expDriftIMG,3)-1
            image2 = uint16(expDriftIMG(:,:,i+1));
            correctedIMG_Exp = imtranslate(image2, [-corrVec(i+1,1),-corrVec(i+1,2)]);
            imwrite(correctedIMG_Exp, exportPathB, 'WriteMode', 'append', 'Compression', 'none');
            if i == nPlanes-1
                disp(strcat('Current Progress: 100%'));
            elseif mod(i,5) == 0
                perc = 100*i/nPlanes;
        ...
        disp('Blue Channel Drift Correction and IMG Export Successful.');
    end

    disp('*******************************************************');
    disp('Drift Data Export Completed.');
    disp('*******************************************************');
end
...
function [out] = nonzeroAVG2(vec1, vec2)

    xVec = [vec1(1:size(vec1,1),1), vec2(1:size(vec2,1),1)];
    yVec = [vec1(1:size(vec1,1),2), vec2(1:size(vec2,1),2)];

    [ii,~,v] = find(xVec);
    outX = accumarray(ii,v,[],@mean);
    
    [ii,~,v] = find(yVec);
    outY = accumarray(ii,v,[],@mean);
    
    out = [outX, outY];

end

function [out] = nonzeroAVG3(vec1, vec2, vec3)

    xVec = [vec1(1:size(vec1,1),1), vec2(1:size(vec2,1),1), vec3(1:size(vec3,1),1)];
    yVec = [vec1(1:size(vec1,1),2), vec2(1:size(vec2,1),2), vec3(1:size(vec3,1),2)];
...
function [out] = nonzeroAVG4(vec1, vec2, vec3, vec4)

    xVec = [vec1(1:size(vec1,1),1), vec2(1:size(vec2,1),1), vec3(1:size(vec3,1),1), vec4(1:size(vec4,1),1)];
    yVec = [vec1(1:size(vec1,1),2), vec2(1:size(vec2,1),2), vec3(1:size(vec3,1),2), vec4(1:size(vec4,1),2)];
